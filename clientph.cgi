...
def getTable(string, genes):
	...
	for i in xrange(0, len(string), 2):
		...
		res += "<td>" + '<a href="redirect.cgi?genes=' + genes + '&phrase=' + re.sub("\*", " ", string[i+1]) + '"> ' + re.sub("\*", " ", string[i+1])+ '</a></td>'
		res += "<td>" + string[i] + "</td>"
		res += "</tr>"
	res += "</table>"
	return res
...
def main():
	...
	if num == "1":
		print '<div style="width:1000px;height:50px;position:absolute;left:0;top:100px;overflow:hidden;background-color:#7A991A"><h1 style="color:#FFFFFF;margin:0;padding:5px;font-family:Verdana,Geneva,sans-serif;position:relative;left:50px">SEARCH</h1><p style="color:#FFFFFF;margin:0;position:absolute;right:10px;top:15px">(' + num + ' gene found.)</p><p><form style="position:absolute;left:500px;top:25%" name = "input" action = "client.cgi" method="GET"><input type="text" id="text" name="genes"><input type="submit" id="submit" value="Submit"></form></p></div>'
	else:
		print '<div style="width:1000px;height:50px;position:absolute;left:0;top:100px;overflow:hidden;background-color:#7A991A"><h1 style="color:#FFFFFF;margin:0;padding:5px;font-family:Verdana,Geneva,sans-serif;position:relative;left:50px">SEARCH</h1><p style="color:#FFFFFF;margin:0;position:absolute;right:10px;top:15px">(' + num + ' genes found.)</p><p><form style="position:absolute;left:500px;top:25%" name = "input" action = "client.cgi" method="GET"><input type="text" id="text" name ="genes"><input type="submit" id="submit" value="Submit"></form></p></div>'
	...
	print '<ul id="sec-nav">'
	print '<li id="sec-navli"><a id="sec-navli" href="client.cgi?genes=' + genes + '">' + 'Table (Cosine)</b></a></li>'
